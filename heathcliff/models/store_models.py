...
from ..mutating.utils import api_put, api_post, set_env
...
class AdGroup(Synchronizable, AppleSerializable):
    ...
    def shallowSave(self, verbose=False):
        if self._id:
            return api_put(
                "campaigns/%s/adgroups/%s" % (self._campaign_id, self._id),
                data=self.__editable_fields(), verbose=verbose)
        else:
            return api_post("campaigns/%s/adgroups" % (self._campaign_id),
                     data=self.__editable_fields(),
                     verbose=verbose)

    def update_adgroup(self, verbose=False):
      data = {
        k: v for k, v in self.__editable_fields().items() if k in {'cpaGoal'}
      }
      return api_put(
          "campaigns/%s/adgroups/%s" % (self._campaign_id, self._id),
          data=data)
...
class Keyword(object):
    ...
    @classmethod
    def update_keywords(cls, keywords, campaignID):
        keywords_exports = {}
        for keyword in keywords:
            if keyword._ad_group_id not in keywords_exports:
                keywords_exports[keyword._ad_group_id] = []
            keywords_exports[keyword._ad_group_id].extend(
                keyword.prepare_for_bulk_export(campaignID,
                                                keyword._ad_group_id))

        responses = []
        for adGroupID, keywords_export in keywords_exports.items():
            data = [
              {k: v for k, v in e.items() if k in {'id', 'status', 'bidAmount'}}
              for e in keywords_export
            ]
            response = api_put("campaigns/{campaignID}/adgroups/{adGroupID}/targetingkeywords/bulk".format(campaignID=campaignID, adGroupID=adGroupID),
                                data=data,
                                verbose=True)
