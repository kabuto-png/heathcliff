...
from ..models.store_models import Campaign, AdGroup, Keyword
...
class SearchAdsAccount(object):
    """
    The search ads API acls endpoint is currently offline, making this class unusable.
    """
    def __init__(self, api_version='v3', limit=2000):
        """
        Initialize the Account API object
        :param api_version: The API version (current is v3)
        """
        self.api_version = api_version
        orgs = self.get_orgs(limit=limit)
        self.apis = [SearchAds.preloaded(org_name=orgs[key], org_id=key) for key in orgs]

    def get_orgs(self, limit=2000):
        """
        Return all Orgs in the account
        :param limit: limit the results (default is 2000)
        :return: a list of Org objects
        """
        raw_acls = api_get("acls", api_version=self.api_version, limit=limit)['data']
        orgs = {raw_acl['orgId']: raw_acl['orgName'] for raw_acl in raw_acls}
        return orgs

class SearchAds(object):
    def __init__(self, org_name, api_version='v3', _fetch=True):
        """
    ...
    @classmethod
    def preloaded(cls, org_name, org_id):
        """
        Create an API object with pre-fetched name and ID
        :param org_name: The organization name as fetched from the API
        :param org_id: The organization ID as fetched from the API
        """
        api = cls(org_name=org_name, _fetch=False)
        api.org_id = org_id
        return api
    ...
    def get_campaigns(self, limit=2000, includeAdGroups=True, includeKeywords=True):
        """
        ...
        for raw_campaign in data:
            campaign = Campaign(**raw_campaign)
            if includeAdGroups:
              campaign.ad_groups = self.get_adgroups(campaignID=campaign._id, limit=limit, includeKeywords=includeKeywords)
    ...
    def get_adgroups(self, campaignID, limit=2000, includeKeywords=True):
        """
        Return all adgroups in the campaign
        :return: a list of AdGroup objects
        """
        adgroups = []
        data = \
            api_get("campaigns/{campaignID}/adgroups".format(campaignID=campaignID),
                    org_id=self.org_id,
                    api_version=self.api_version,
                    limit=limit)['data']
        for raw_adgroup in data:
            adgroup = AdGroup(**raw_adgroup)
            if includeKeywords:
              adgroup.keywords = self.get_keywords(campaignID=campaignID, adGroupID=adgroup._id, limit=limit)
            adgroups.append(adgroup)
        return adgroups

    def get_keywords(self, campaignID, adGroupID, limit=2000):
        """
        Return all keywords in the ad group
        :return: a list of Keyword objects
        """
        keywords = []
        data = \
            api_get("campaigns/{campaignID}/adgroups/{adGroupID}/targetingkeywords".format(campaignID=campaignID, adGroupID=adGroupID),
                    org_id=self.org_id,
                    api_version=self.api_version,
                    limit=limit)['data']
        for raw_keyword in data:
            keywords.append(Keyword(**raw_keyword))
        return keywords
